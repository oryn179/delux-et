...
export function PropertyCard({
  ...
  const handleCompareClick = (e: React.MouseEvent) => {
    ...
    if (isInCompare(id)) {
      ...
      toast({ title: "Limit reached", description: "You can only compare up to 4 properties", variant: "destructive" });
    }
  };
  ...
  const handleContact = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!isAuthenticated) {
      toast({ title: "Login Required", description: "Please sign in to contact the owner." });
      navigate("/signin");
      return;
    }
    navigate(`/property/${id}`);
    ...
    <div className="group bg-card rounded-2xl overflow-hidden shadow-card border border-border hover:shadow-elevated transition-all duration-300">
      <div className="relative aspect-[4/3] overflow-hidden cursor-pointer" onClick={handleViewDetails}>
        ...
        <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button variant="secondary" size="icon" className="rounded-full h-9 w-9" onClick={handleFavoriteClick}>
            <Heart className={`h-4 w-4 ${isFavorite(id) ? "fill-primary text-primary" : ""}`} />
          </Button>
          <Button variant="secondary" size="icon" className={`rounded-full h-9 w-9 ${isInCompare(id) ? "bg-primary text-primary-foreground" : ""}`} onClick={handleCompareClick}>
      ...
      <div className="p-4 space-y-3">
        ...
        <h3 className="font-semibold text-lg line-clamp-1 group-hover:text-primary transition-colors cursor-pointer" onClick={handleViewDetails}>
          {title}
        </h3>
        ...
        <div className="flex gap-2 pt-2">
          <Button variant="outline" size="sm" className="flex-1" onClick={handleViewDetails}>View Details</Button>
          <Button size="sm" className="flex-1 gradient-primary border-0" onClick={handleContact}>Contact</Button>
        </div>
      </div>
    </div>
  );
